...
Каждая основная карточка получает круглую кнопку `i`. Для ключевых механик используется конкретный текст, для остальных карточек — единый безопасный шаблон чтения стоимости, требований, нагрузки и эффекта. Обучение показывается один раз и доступно повторно из меню.
