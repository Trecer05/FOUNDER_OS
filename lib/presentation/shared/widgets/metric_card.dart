...
String? _metricHint(String label) {
  ...
  if (key.contains('оценк')) {
    return 'Расчётная стоимость всей компании. Учитывает выручку, прибыль, пользователей, качество продуктов, технологии, портфель и экосистему. Это не деньги на счёте.';
  }
  if (key.contains('доля основателя') || key.contains('founder share')) {
    return 'Процент компании, который принадлежит основателю. Инвестиционные раунды размывают долю; ниже 50% основатель теряет контроль и сессия заканчивается.';
  }
  if (key.contains('загрузка сервер') || key.contains('server load')) {
    return 'Отношение требуемой продуктами мощности к реально выделенному compute. До 85% — запас, 85–100% — риск, выше 100% начинаются деградации.';
  }
  if (key.contains('compute') || key.contains('мощност')) {
    return 'Вычислительный ресурс инфраструктуры. Продукты потребляют его пропорционально аудитории, стеку, функциям и AI-нагрузке.';
  }
