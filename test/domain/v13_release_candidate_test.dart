...
void main() {
  ...
  test('million-ruble premium campaign has a meaningful prelaunch forecast', () {
  ...
  test('technical improvements are fast enough and overload navigation is free', () {
    ...
    for (final type in ProductImprovementType.values) {
      expect(initial.improvementRequiredHours('fixture', type), lessThan(250));
      expect(ProductEvolutionCatalog.improvementByType(type).baseCost, greaterThan(0));
    }
  ...
  test('rival landscape is billion-scale and final acquisition respects 70 percent gate', () {
    ...
    expect(
      GameCatalog.marketCompanies.map((item) => item.users).reduce((a, b) => a > b ? a : b),
    ...
    for (final company in GameCatalog.marketCompanies.where((item) => item.id != last.id)) {
