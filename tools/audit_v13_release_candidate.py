...
# Repository-facing source should not contain obvious assistant-attribution traces.
trace_pattern = re.compile(r"generated by (chatgpt|claude|codex)|as an ai language model|openai codex", re.I)
for folder in ("lib", "test", "README.md"):
    target = ROOT / folder
    files = [target] if target.is_file() else list(target.rglob("*.dart"))
    for file in files:
        if trace_pattern.search(file.read_text(encoding="utf-8")):
            errors.append(f"assistant attribution trace: {file.relative_to(ROOT)}")
