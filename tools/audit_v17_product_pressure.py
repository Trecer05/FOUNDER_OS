...
checks: list[tuple[str, str, str]] = [
    ...
    ('lib/domain/simulation/engine/game_engine.dart', 'final satisfaction = state.productUserSatisfaction(product) / 100;', 'R16 churn shock uses satisfaction'),
    ('lib/domain/simulation/engine/game_engine.dart', 'final trust = product.brandTrust.clamp(0, 1).toDouble();', 'R16 churn shock uses trust'),
    ('lib/domain/simulation/engine/game_engine.dart', '(1 - satisfaction) * 0.018', 'R16 churn shock severity follows bad experience'),
    ...
    ('lib/presentation/features/products/product_workspace_screen.dart', 'Навязчивость рекламы', 'clear ads control'),
    ('lib/presentation/features/products/product_workspace_screen.dart', "'Удовлетворённость'", 'R16 monetization satisfaction preview visible'),
    ('lib/presentation/features/products/product_workspace_screen.dart', "'Конверсия в оплату'", 'R16 paid conversion preview visible'),
    ('lib/presentation/features/products/product_workspace_screen.dart', "'Платящих / монетизируемых'", 'R16 paying users preview visible'),
    ('lib/presentation/features/products/product_workspace_screen.dart', "'Ожидаемая выручка'", 'R16 monetization revenue preview visible'),
